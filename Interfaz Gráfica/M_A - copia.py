...
class MainWindowLogic(QtWidgets.QMainWindow):
    ...
    def play_video(self, filename):
        ...
        # Convertir ruta a formato file:/// para VLC
        clean_path = os.path.abspath(path).replace("\\", "/")
        media = self.vlc_instance.media_new(f"file:///{clean_path}", "demux=any")
        self.vlc_player.set_media(media)

        # Asegurarse de que el widget tenga winId antes de set_hwnd
        ...
        def monitor():
            ...
            while self.vlc_player.is_playing():
                time.sleep(0.5)
            QtCore.QTimer.singleShot(0, self.on_video_finished)
